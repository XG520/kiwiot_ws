...
class TokenManager:
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry):
        ...
        self._lock = Lock()
        self._storage_file = Path(hass.config.path("kiwiot_tokens.json"))
    ...
    async def get_token(self, session: aiohttp.ClientSession) -> str:
        ...
        async with self._lock:
            ...
            try:
                if not self._is_token_expired() and await self.is_token_valid(session):
                ...
                if self._refresh_token:
                    try:
                        ...
                        _LOGGER.warning(f"刷新token失败，尝试重新获取: {e}")

                await self._fetch_new_token(session)
                return self._access_token

            except Exception as e:
                _LOGGER.info(f"获取token失败: {e}")
                raise
    ...
    async def _refresh_access_token(self, session: aiohttp.ClientSession) -> None:
        ...
        async with session.post(f"{BASE_URL}/restapi/oauth/token", headers=headers, json=data) as response:
    ...
    async def _fetch_new_token(self, session: aiohttp.ClientSession) -> None:
        """获取新的token"""
        headers = {"X-Kiwik-Client-Id": self._client_id}
        data = {
            "identifier": self._identifier,
        ...
        async with session.post(f"{BASE_URL}/restapi/auth/tokens", headers=headers, json=data) as response:
            response_data = await response.json()
            
            if response.status != 201:
                raise ValueError(f"获取token失败: {response_data}")
    ...
    async def is_token_valid(self, session: aiohttp.ClientSession) -> bool:
        """验证当前token是否有效"""
