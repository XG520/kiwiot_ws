...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    try:
        ...
        # 添加实体
        async def add_entities(component: EntityComponent):
            await component.async_add_entities([user_entity, group_entity])

            # 获取设备实体并添加
            device_entities = await group_entity.get_device_entities()
            await component.async_add_entities(device_entities)

        hass.async_create_task(add_entities(hass.data[DOMAIN]["entity_component"]))
    except Exception as e:
